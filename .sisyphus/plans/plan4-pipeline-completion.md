...
- [ ] T4. **实现 OpenCVMaskRefiner（遮罩优化）**

  **What to do**:
  - 新建 `translation/mask/OpenCVMaskRefiner.kt`，实现 `MaskRefiner` 接口
  - 在 `prepare()` 中调用 `ensureOpenCVLoaded()`，失败则抛 `IllegalStateException`
  - `refine()` 实现：
    - rawMask 为 null → 创建全零 Bitmap 作为初始 mask
    - Bitmap → Mat 转换（使用 `OpenCVUtils.bitmapToMat`）
    - 将 mask 二值化（阈值 127 → 0 或 255）
    - 用 `kernelSize` 和 `dilationOffset` 参数计算膨胀迭代次数：`iterations = dilationOffset / kernelSize`
    - 对每个 `textRegions[n].minRect` 区域，在 mask 上用 `Imgproc.rectangle` 绘制白色块（扩展文本区域）
    - 用 `Imgproc.dilate(mask, kernel)` 做形态学膨胀（kernel 用 `Imgproc.getStructuringElement(MORPH_ELLIPSE, Size(kernelSize, kernelSize))`）
    - Mat → Bitmap 转换返回
  - 编写 `OpenCVMaskRefinerTest`（androidTest）：
    - 测试已知矩形 mask → 膨胀后 white pixels 数量 ≥ 原始 white pixels 数量
    - 测试 null rawMask → 创建非空 Bitmap
    - 测试空 textRegions → 返回全零 mask

  **Must NOT do**:
  - 不实现气泡检测（Python 的 `ignore_bubble`，P2 后续）
  - 不修改 MaskRefiner 接口签名

  **Recommended Agent Profile**:
  - **Category**: `deep` — OpenCV 形态学操作需要仔细的 Mat 生命周期管理
  - **Skills**: []

  **Parallelization**:
  - **Can Run In Parallel**: YES (Wave 2, 与 T5/T6/T7 并行)
  - **Blocks**: T12（管线端到端测试）
  - **Blocked By**: T1（需要 DI 就绪）

  **References**:
  - `translation/api/MaskRefiner.kt:1-15` — 接口定义，输入输出签名
  - `translation/util/OpenCVUtils.kt:13-18` — `ensureOpenCVLoaded()` 和 `bitmapToMat`/`matToBitmap` 工具
  - `translation/data/TextBlock.kt:1-24` — TextBlock 数据类（注意 T8 会修复 minRect/center 存根）
  - `translation/util/GeometryUtils.kt` — 几何工具函数

  **Acceptance Criteria** (TDD):
  - [ ] 测试文件: `app/src/androidTest/.../translation/mask/OpenCVMaskRefinerTest.kt`
  - [ ] `./gradlew :app:connectedAndroidTest --tests "*OpenCVMaskRefinerTest"` → PASS (3+ tests)
  - [ ] 编译通过

  **QA Scenarios (TDD)**:
  ```
  Scenario: Dilation increases white pixel count on known mask
    Tool: AndroidJUnit4 (instrumented)
    Preconditions: 100x100 Bitmap, 50x50 white rectangle at (25,25,75,75) as rawMask
    Steps:
      1. val refiner = OpenCVMaskRefiner()
      2. refiner.prepare()
      3. val result = refiner.refine(emptyList(), bitmap, rawMask, kernelSize=5, dilationOffset=10)
      4. 统计 result 中的白色像素数
      5. 断言 resultWhitePixels > rawMaskWhitePixels（膨胀生效）
      6. 断言 result.width == 100 && result.height == 100
      7. refiner.release()
    Expected Result: 输出 mask 中 white pixels 数量显著增加
    Failure Indicators: resultWhitePixels == rawMaskWhitePixels（膨胀未生效）
    Evidence: .sisyphus/evidence/plan4-t4-dilation.txt

  Scenario: Null rawMask creates fresh mask
    Tool: AndroidJUnit4
    Steps:
      1. val result = refiner.refine(emptyList(), bitmap, rawMask=null, kernelSize=3, dilationOffset=0)
      2. 断言 result != null
      3. 断言 result.width == bitmap.width
    Expected Result: 不崩溃，返回全零 Bitmap
    Evidence: .sisyphus/evidence/plan4-t4-nullmask.txt
  ```

  **Commit**: YES
  - Message: `feat(mask): implement OpenCVMaskRefiner with morphological dilation`
  - Files: `OpenCVMaskRefiner.kt`, `OpenCVMaskRefinerTest.kt`

- [ ] T5. **实现 SimpleFillInpainter（图像修复）**

  **What to do**:
  - 新建 `translation/inpaint/SimpleFillInpainter.kt`，实现 `Inpainter` 接口
  - 在 `prepare()` 中调用 `ensureOpenCVLoaded()`
  - `inpaint()` 实现：
    - Bitmap → Mat 转换
    - 遍历 mask 中的白色像素（值 > 127），将对应 bitmap 像素改为 `Scalar(255, 255, 255)`（纯白）
    - 返回填充后的 Bitmap
    - 如果 mask 全透明（无白色像素）→ 直接返回 bitmap
    - 如果 mask 和 bitmap 尺寸不一致 → resize mask 到 bitmap 尺寸
  - 编写 `SimpleFillInpainterTest`（androidTest）：
    - 测试 50x50 白色 mask → 对应区域变成白色
    - 测试全透明 mask → bitmap 不变
    - 测试 mask 覆盖全图 → 返回全白 bitmap

  **Must NOT do**:
  - 不实现 LaMa ONNX 修复（P2 后续）
  - 不实现背景色检测（用纯白色，Python 的 NoneInpainter 行为）

  **Recommended Agent Profile**:
  - **Category**: `quick` — 简单的像素级填充操作
  - **Skills**: []
  ...
  **References**:
  - `translation/api/Inpainter.kt:1-13` — 接口定义
  - `translation/util/OpenCVUtils.kt` — Mat 转换工具
  - `translation/data/config/InpainterConfig.kt` — config（注意 `inpaintingSize` 在此简化实现中不强制使用）

  **Acceptance Criteria** (TDD):
  - [ ] 测试文件: `app/src/androidTest/.../translation/inpaint/SimpleFillInpainterTest.kt`
  - [ ] `./gradlew :app:connectedAndroidTest --tests "*SimpleFillInpainterTest"` → PASS (3+ tests)

  **QA Scenarios (TDD)**:
  ```
  Scenario: Masked region filled white
    Tool: AndroidJUnit4
    Preconditions: 100x100 Bitmap (all blue), 50x50 mask at (0,0,50,50) (white)
    Steps:
      1. val inpainter = SimpleFillInpainter()
      2. inpainter.prepare()
      3. val result = inpainter.inpaint(blueBitmap, whiteMask, InpainterConfig())
      4. 检查 result.getPixel(10,10) → 断言 == Color.WHITE
      5. 检查 result.getPixel(60,60) → 断言 == Color.BLUE（未遮罩区域不变）
      6. inpainter.release()
    Expected Result: 遮罩区域变白，非遮罩区域保持蓝色
    Evidence: .sisyphus/evidence/plan4-t5-fill.txt

  Scenario: Fully transparent mask returns original
    Tool: AndroidJUnit4
    Steps:
      1. val result = inpainter.inpaint(bitmap, transparentMask, config)
      2. 断言 result.sameAs(bitmap) == true
    Expected Result: 输出与原图相同
    Evidence: .sisyphus/evidence/plan4-t5-transparent.txt
  ```

  **Commit**: YES
  - Message: `feat(inpaint): implement SimpleFillInpainter filling masked regions with white`
  - Files: `SimpleFillInpainter.kt`, `SimpleFillInpainterTest.kt`

- [ ] T6. **实现 HorizontalTextRenderer（横排文字渲染）**

  **What to do**:
  - 新建 `translation/render/HorizontalTextRenderer.kt`，实现 `TextRenderer` 接口
  - 构造函数接收 `Context`（访问 assets 中的字体文件）
  - `prepare()` 中加载字体：`Typeface.createFromAsset(context.assets, "fonts/NotoSansCJK-Regular.ttf")`
  - `render()` 实现：
    - 对每个 `textRegions` 中 `translation` 非空的 TextBlock：
      a. 计算目标字号：`targetFontSize = region.fontSize + config.fontSizeOffset`，限制在 `fontSizeMinimum` 之上
      b. 用 `Paint` 计算文字宽度（`paint.measureText(text)`），如超过 region 宽度则缩小字号
      c. 创建临时 Canvas，在 region 的 `minRect` 位置绘制文字
      d. 应用 `Paint` 样式：颜色（fgColor 或 Color.BLACK）、描边（`disableFontBorder` 控制）、对齐（LEFT/CENTER/RIGHT）
    - 返回渲染后的 Bitmap（在新 Canvas 上绘制，不修改原图）
  - 编写 `HorizontalTextRendererTest`（androidTest）：
    - 测试文字出现在区域内的像素
    - 测试空 textRegions 返回原图
    - 测试空 translation 跳过该 region

  **Must NOT do**:
  - 不实现竖排 CJK 渲染
  - 不实现透视变换（`Matrix.setPolyToPoly`）— 先用简单的矩形定位
  - 不实现字号自适应算法（只做最基础的字号调整）

  **Recommended Agent Profile**:
  - **Category**: `deep` — Canvas/Paint 渲染涉及字体加载、尺寸计算、像素级绘制
  - **Skills**: []

  **Parallelization**:
  - **Can Run In Parallel**: YES (Wave 2)
  - **Blocks**: T12
  - **Blocked By**: T1, T7（需要字体资产）

  **References**:
  - `translation/api/TextRenderer.kt:1-14` — 接口定义
  - `translation/data/TextBlock.kt:1-24` — TextBlock 含 fontSize, fgColor, bgColor, alignment, minRect
  - `translation/data/config/RendererConfig.kt:1-16` — 配置含 fontSizeOffset, fontSizeMinimum, disableFontBorder
  - Android 官方文档: `android.graphics.Paint`, `android.graphics.Canvas`, `android.graphics.Typeface`

  **Acceptance Criteria** (TDD):
  - [ ] 测试文件: `app/src/androidTest/.../translation/render/HorizontalTextRendererTest.kt`
  - [ ] `./gradlew :app:connectedAndroidTest --tests "*HorizontalTextRendererTest"` → PASS (3+ tests)

  **QA Scenarios (TDD)**:
  ```
  Scenario: Text rendered at text block position
    Tool: AndroidJUnit4
    Preconditions: 200x200 Bitmap, 1 TextBlock with minRect=(10,10,180,40), translation="Test"
    Steps:
      1. val renderer = HorizontalTextRenderer(context)
      2. renderer.prepare()
      3. val result = renderer.render(bitmap, listOf(textBlock), RendererConfig())
      4. 断言 result.getPixel(50,30) != bitmap.getPixel(50,30)（该区域发生了变化）
      5. 断言 result.width == 200 && result.height == 200（尺寸不变）
      6. renderer.release()
    Expected Result: TextBlock 区域内的像素发生变化（文字被绘制）
    Failure Indicators: result.sameAs(bitmap)（文字未绘制）
    Evidence: .sisyphus/evidence/plan4-t6-render.txt

  Scenario: Empty text regions return original
    Tool: AndroidJUnit4
    Steps:
      1. val result = renderer.render(bitmap, emptyList(), config)
      2. 断言 result.sameAs(bitmap) == true
    Expected Result: 输出与原图相同
    Evidence: .sisyphus/evidence/plan4-t6-empty.txt
  ```

  **Commit**: YES
  - Message: `feat(render): implement HorizontalTextRenderer with Canvas/Paint text drawing`
  - Files: `HorizontalTextRenderer.kt`, `HorizontalTextRendererTest.kt`

- [ ] T7. **添加 Noto Sans CJK 字体资产**
